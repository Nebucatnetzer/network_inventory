...
def test_device_detail_view_net_relation(create_admin_user):
    ...
    device_in_net1 = mixer.blend(
        "devices.DeviceInNet", device=device, net=net1, ip="10.7.89.100"
    )
    device_in_net2 = mixer.blend(
        "devices.DeviceInNet", device=device, net=net2, ip="10.8.89.100"
    )
    client = Client()
...
def test_device_detail_view_net_dhcp_relation(create_admin_user):
    ...
    assert response.status_code == 200 and helper.in_content(
        response, device_in_net1.ip_status
    )
